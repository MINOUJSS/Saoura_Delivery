      ...
      <div class="box box-primary">
        <div class="box-header with-border">
          <h3 class="box-title">إضافة تحت تحت صنف</h3>
                    ...
                    <!-- form start -->
                  <form role="form" action="{{route('admin.sub_sub_category.store')}}" method="POST" enctype="multipart/form-data">
                      @csrf 
                      <div class="col-lg-6">                            
                      <div class="box-body">
                        <div class="form-group {{$errors->has('sub_category')? 'has-error':''}}">
                          <label for="sub_category">إختر تحت الصنف الذي ينتمي إليه تحت تحت الصنف</label>
                            ...
                            <option value="{{$sub_category->id}}" @if(old('sub_category')==$sub_category->name) selected @endif >{{$sub_category->name}}</option>
                              @endforeach
                            </select>
                        ...
                        <input name="sub_sub_category" type="text" class="form-control" id="sub_sub_category" placeholder="أدخل إسم تحت الصنف هنا" value="{{old('sub_sub_category')}}">
                          ...
                          <div class="box-body no-padding">
                            <table class="table table-condensed">
                              ...
                              <td>{{$sub_sub_category->created_at}}</td>
                                <td><a href="{{url('admin/sub-sub-category').'/'.$sub_sub_category->id.'/edit'}}" style="margin-left:20px;"><i class="fa fa-edit text-success"></i></a><i id="delete_category" title="{{$sub_sub_category->name}}" url="{{url('admin/sub-sub-category').'/'.$sub_sub_category->id.'/delete'}}" class="fa fa-trash text-danger cursor-pointer"></i></td>
                              </tr>
                              @endforeach
                              @else 
                              <tr>
